import { useAnimateInView } from "@/hooks/useAnimate";
...
const GoWebServer = () => {
  ...
  return (
    <div className="flex h-fit w-full flex-shrink-0 flex-col items-center justify-center py-100">
      <div
        ref={scope}
        className="flex w-full flex-col items-start justify-center gap-24 pb-100"
      >
        <div>
          <div
            id="Title"
          ...
          >
            ...
            <h1 className="text-3xl font-semibold">Web Server</h1>
          </div>
          <p data-animate style={{ opacity: 0 }} className="text-neutral-70">
            A Golang backend using CoinMarketCap API to fetch Crypto and
            Blockchain data.
          </p>
          <p data-animate style={{ opacity: 0 }} className="text-neutral-70">
            CI/CD using Github Actions and Docker containers.
          </p>
        </div>
        <Button
          data-animate
        ...
        >
          ...
          <a href="/colors" className="text-muted hover:text-black">
            Backend Repository
          </a>
        </Button>
      </div>

      <div className="image gradient-225 aspect-video w-full overflow-clip rounded-[24px]">
        <img src="/backend.webp" alt="" className="w-full" />
      </div>
    </div>
  );
};

export default GoWebServer;
